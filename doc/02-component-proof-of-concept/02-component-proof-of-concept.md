...
I am choosing the music playlist because the other two may become complicated with all of the math. It may be difficult to implement some arithmetic-based methods for the RGB color and bank account components, especially if I use `int`, which will cause some rounding errors. The music playlist will be much easier to implement because the methods are a lot simpler and it functions similarly to a queue
